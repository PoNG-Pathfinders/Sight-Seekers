...
public class PlayerLocationService : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
       while (!UnityEditor.EditorApplication.isRemoteConnected && hasMobile)
        {
            yield return null;
        }
#endif
        ...
        if (!Input.location.isEnabledByUser)
        {
            ...
            //NOTE: If location is not enabled, we originally initialized the postion of the player to somewhere in Los Angeles, just for demonstration purposes, but now it's set to the PoNG Center at UCSD.
            loc.setLatLon_deg(32.8815f, -117.241191f);
