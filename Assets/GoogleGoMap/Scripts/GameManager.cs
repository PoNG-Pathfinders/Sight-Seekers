...
using static GoogleStaticMap;
using static GoogleStaticMap.MapRectangle.GetCorner;
using static GoogleStaticMap.MapRectangle;
...
public class GameManager : Singleton<GameManager> {
    ...
    void Update () {
		...
		MyPoint tileCenterMercator = getOuterMap(mainMap).tileCenterMercator(playerGeoPosition);
    ...
    private MyPoint newCenterMercator(GoogleStaticMap map, GetCorner corner)
    {
    ...
    private MyPoint newCenterMercator(GoogleStaticMap map, GetCorner cornerOne, GetCorner cornerTwo)
    {
        Vector2 centerVec = map.getPositionOnMap(map.centerLatLon);
